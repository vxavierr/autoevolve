...
claude --dangerously-skip-permissions --model sonnet --verbose -p "Run /autoevolve --auto. This is an autonomous run — no human in the loop. For code domain: only safe changes (tests, lint fixes). For prompts: optimize without losing quality. For behavior: analyze and propose guardrails. For flow: scan and report only. Commit each change with [autoevolve] prefix. Stop at plateau (3 consecutive no-improvement iterations). No max iterations — trust plateau detection to stop. Write summary to .autoevolve/reports/." 2>&1
